...
std::string tobinary(unsigned int num) {
	...
	result += num == 0 ? '0' : '1';
	std::reverse(result.begin(), result.end()); // функция, принимающая указатели на начало и на конец строки и отражающая содержимое строки
	return result;
}

bool bit_at_index(unsigned int num, int index) {
	//Cложный вариант решения
...
int main () {
	...
	//Битовые операции
	/*char sym1 = 1;
	std::cout << chui(sym1 << 369875) << std::endl;
	char sym2 = 44;
	std::cout << chui(sym2 >> 3) << std::endl;
	...
	std::cin >> index;
	std::cout << "Бит в позиции: " << bit_at_index(n, index) << std::endl;
